...
if ( ! class_exists( 'jayp_Customize' ) ) :
	class jayp_Customize {
		...
		public static function register( $wp_customize ) {
			...
			$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'jayp_accent_color', array(
				'label' 		=> __( 'Accent Color', 'jayp' ),
				'section' 		=> 'colors',
			...
			// Only display the Customizer section for the jayp_logo setting if the setting already has a value.
			...
			if ( get_theme_mod( 'jayp_logo' ) ) {

				$wp_customize->add_section( 'jayp_logo_section', array(
					'title'       => __( 'Logo', 'jayp' ),
					'priority'    => 40,
					'description' => __( 'Upload a logo to replace the default site name and description in the header','jayp' ),
				) );

				$wp_customize->add_setting( 'jayp_logo', array(
					'sanitize_callback' => 'esc_url_raw'
				) );

				$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'jayp_logo', array(
					'label'    => __( 'Logo', 'jayp' ),
					'section'  => 'jayp_logo_section',
					'settings' => 'jayp_logo',
				) ) );
		...
		public static function custom_css_output() {
			...
			$properties = apply_filters( 'jayp_accent_color_elements', array(
	...
	add_action( 'customize_register', array( 'jayp_Customize', 'register' ) );

	// Output custom CSS to live site
	add_action( 'wp_head', array( 'jayp_Customize', 'custom_css_output' ) );
